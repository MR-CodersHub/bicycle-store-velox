  ...
  function initProductList() {
    ...
    function matches(p) {
      if (state.cat !== 'all') {
        ...
        if (c === 'sale') {
          if (p.badge !== 'sale' && !p.compareAt) return false;
        } else if (c === 'road') {
          if (pCat !== 'road' && pType !== 'road') return false;
        } else if (c === 'mountain') {
          if (pCat !== 'mountain' && pType !== 'mountain') return false;
        } else if (c === 'hybrid') {
          if (pCat !== 'hybrid' && pType !== 'hybrid') return false;
        } else if (c === 'electric') {
          if (pCat !== 'electric' && pType !== 'electric') return false;
        } else if (c === 'kids') {
          if (pCat !== 'kids' && pType !== 'kids') return false;
        } else if (c === 'accessories') {
          if (pCat !== 'accessories') return false;
        } else if (c === 'sports') {
          if (pCat !== 'road' && pCat !== 'mountain') return false;
        } else if (c === 'men' || c === 'women') {
          // graceful fallback for footer links
          if (pCat === 'kids') return false;
        } else {
          if (pCat !== c && pType !== c) return false;
        }
      }
      var q = state.q.trim().toLowerCase();
      if (!q) return true;
      var kw = Array.isArray(p.keywords) ? p.keywords.join(' ') : (p.keywords || '');
      return (p.name + ' ' + p.brand + ' ' + p.category + ' ' + (p.type || '') + ' ' + kw).toLowerCase().indexOf(q) !== -1;
    }
