...
var STRIDE_DATA = (function () {
  ...
  /* Resolve local asset paths for GitHub Pages subfolder hosting:
     '/assets/x.jpg' works only when the site is served from a domain
     root, so rewrite those to relative paths (./ or ../) per page. */
  for (var i = 0; i < PRODUCTS.length; i += 1) {
    if (PRODUCTS[i].image && PRODUCTS[i].image.indexOf('/assets/') === 0) {
      PRODUCTS[i].image = assetPrefix + PRODUCTS[i].image.slice(1);
    }
  }
